...
`FULL` joins are not yet implemented and are on our roadmap.
...
`INNER` and `LEFT` `JOIN` have a mandatory `ON` clause allowing arbitrary
join predicates:

```questdb-sql
[INNER | LEFT] JOIN { table | (subQuery) }
...
`HORIZON JOIN` and `WINDOW JOIN` are specialized time-series joins with their
own dedicated syntax - see [HORIZON JOIN](/docs/query/sql/horizon-join/) and
[WINDOW JOIN](/docs/query/sql/window-join/) for details.
