...
- [SHOW](/docs/query/sql/show/) - `SHOW USERS`, `SHOW GROUPS`, and `SHOW SERVICE ACCOUNTS`, including their filtered forms and the new `SHOW SERVICE ACCOUNTS groupName` filter, gain a trailing `memory_limit` column. Clients that read these results by position need updating
