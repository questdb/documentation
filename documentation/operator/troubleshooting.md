...
description: Diagnose Operator and QuestDB cluster conditions, events, logs, and object-store configuration.
---
...
| --- | --- | --- |
| `ConfigRejected=True` | An engine setting is invalid, injection-prone, or operator-owned. | Read reason/message; remove the rejected key from `spec.config` or `spec.replication.config`. Compare with [Configuration](/docs/operator/configuration/#extra-engine-options). |
...
| `PromotionRequired=True` | Established primary PVC is missing/Terminating; the operator fences its Pod, refuses an empty replacement, preserves `currentPrimary`, and does not auto-promote. | Confirm PVC/PV/cloud-disk and zero ready RW endpoints. Restore the volume or select a replica and follow [promotion and failover](/docs/operator/high-availability/#promotion-and-failover), accepting emergency loss when a drain is impossible. |
| `Recovered=True` | Engine reported restore completion. | Still validate actual tables, timestamps, and row counts before traffic. |
| `RecoveryFailed=True` | Restore init/runtime failed or was incomplete. | Read reason/message and genesis init logs; verify source store/root/instance/target. `spec.bootstrap` is immutable, so correct it in a fresh destination. |
...
If the cause remains unclear, collect the [support bundle](/docs/operator/support/#collect-a-support-bundle) before changing the failing resources.
