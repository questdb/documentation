...
| --- | --- | --- |
| `ConfigRejected=True` | An engine setting is invalid, injection-prone, or operator-owned. | Read reason/message; remove the rejected key from `spec.config` or `spec.replication.config`. Compare with [Configuration](/docs/enterprise-kubernetes-operator/configuration/#extra-engine-options). |
