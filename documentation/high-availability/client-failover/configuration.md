...
These keys apply to every WS / WSS / HTTP / HTTPS client. They are documented
in full on the
...
| `zone` | string | unset | Client's zone identifier (opaque, case-insensitive — `eu-west-1a`, `dc-amsterdam`, etc.). Egress prefers same-zone peers when `target` is `any` or `replica`. Silently ignored on ingress. |
| `target` | `any` \| `primary` \| `replica` | `any` | Which server role the client accepts. See [Role filter](/docs/high-availability/client-failover/concepts/#role-filter-target) for the role table. |
...
```java
try (QueryClient client = QueryClient.fromConfig(
        "ws::addr=replica-eu-1a:9000,replica-eu-1b:9000,primary:9000;"
        + "zone=eu-west-1a;target=any;")) {
    try (ResultSet rs = client.execute("SELECT * FROM trades WHERE ts > now() - 1h")) {
        // ...
    }
}
```
