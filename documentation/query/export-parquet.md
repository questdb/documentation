...
By default, Parquet files generated by QuestDB are compressed using `zstd` compression. One of the key advantages of Parquet
over QuestDB’s native format is its built-in compression.

You can configure a custom compression codec and compression level in `server.conf` with:

```ini
# ZSTD, LZ4_RAW, or SNAPPY
cairo.partition.encoder.parquet.compression.codec=ZSTD
# for ZSTD: level is from 1 to 22, 1 is fastest, 9 is default
