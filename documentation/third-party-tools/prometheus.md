...
| `questdb_jvm_unknown_gc_count_total`              | counter | Number of times JVM garbage collection of unknown type was triggered. Usually zero, some non-mainstream JVM implementations may show non-zero.  |
| `questdb_jvm_unknown_gc_time_total`               | counter | Total time spent on JVM garbage collection of unknown type in milliseconds. Usually zero, some non-mainstream JVM implementations may show non-zero. |
