...
When dropping a table, permissions on it are preserved by default (useful if the
table is recreated). Use `DROP TABLE ... CASCADE PERMISSIONS` to also remove all
