...
about it, so revoking a token in Entra ID does not end the access it grants:
the token keeps working until it expires. Read that option before going to
production, and note that clients must send the ID token here, not the access
token.
