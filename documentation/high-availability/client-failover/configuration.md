...
try (Sender sender = Sender.fromConfig(
        "wss::addr=node-a:9000,node-b:9000,node-c:9000;sf_dir=/var/lib/qdb-sender;")) {
    sender.table("events")
...
The `wss::` scheme picks the QWP WebSocket transport over TLS — multi-host
failover requires QuestDB Enterprise, which serves TLS, so Enterprise examples
use `wss::` rather than plain `ws::`. `sf_dir` enables the
...
try (QwpQueryClient client = QwpQueryClient.fromConfig(
        "wss::addr=replica-eu-1a:9000,replica-eu-1b:9000,primary:9000;"
...
The `async` initial-connect policy lets the producer thread proceed
immediately. Note the 30-minute `reconnect_max_duration_millis` is **inert
here** — it bounds only a *blocking* (`sync`) initial connect. Under `async`
the I/O thread retries indefinitely, and how long an outage you survive is set
by `sf_max_total_bytes` against available disk, not by this key.

### Tight egress failover for an interactive dashboard

```java
try (QwpQueryClient client = QwpQueryClient.fromConfig(
        "wss::addr=node-a:9000,node-b:9000;"
