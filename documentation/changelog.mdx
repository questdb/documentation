...
- [Kafka connector](/docs/connect/message-brokers/kafka/) - Rewritten around the QWP transport (`ws::` / `wss::`): acknowledged delivery with offsets committed only after QuestDB confirms the rows, the `qwp.*` delivery options, the client keys the connector manages, outage and rebalance behaviour, schema-mismatch isolation to the dead letter queue, backpressure tuning, the experimental raw JSON fast path, and a legacy ILP section with migration steps
