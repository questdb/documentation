...
description:
  How QuestDB stores and exports data in Apache Parquet format. Covers creating
  tables directly as Parquet, converting partitions in place manually or
  automatically, and exporting query results as external Parquet files.
---

QuestDB works with [Apache Parquet](https://parquet.apache.org/) in several ways:

- A table can be **created directly in Parquet format**, so its partitions are
  stored as Parquet from the start.
- Existing partitions can be **converted in place**, automatically with a storage
  policy (QuestDB Enterprise) or manually with `ALTER TABLE`, while the data stays
  managed by QuestDB.
- Query results and whole tables can be **exported** as external Parquet files
...
A partitioned [WAL](/docs/concepts/write-ahead-log/) table can store its
partitions as Parquet by default with the `FORMAT PARQUET` clause, rather than
converting them later:

```questdb-sql title="Create a table with Parquet partitions"
CREATE TABLE trades (
  timestamp TIMESTAMP,
  symbol SYMBOL,
  price DOUBLE,
...
for the clause, and
[ALTER TABLE SET FORMAT](/docs/query/sql/alter-table-set-format/) to switch an
...
In-place conversion turns existing partitions from native format into Parquet
(or back) while they remain under QuestDB's control and can still be queried as
if they were in native format.
...
In QuestDB Enterprise, conversion is fully automated with
[storage policies](/docs/concepts/storage-policy/). A policy converts partitions
to Parquet on a schedule as they age, and can also drop the native files
afterwards.

A policy can be attached when creating the table with the `STORAGE POLICY`
clause on [`CREATE TABLE`](/docs/query/sql/create-table/#storage-policy), or set
later with
...
```questdb-sql title="Convert to Parquet after 1 hour (Enterprise)"
...
[`DETACH` command](/docs/query/sql/alter-table-detach-partition/).
...
Query results and whole tables can be exported as external Parquet files. Unlike
in-place conversion, the exported files are written outside QuestDB (streamed to
the client or written to the server filesystem) for use in other tools such as
DuckDB, Pandas, or Polars.
...
or using the [`/api/v1/sql/execute` endpoint](/docs/connect/compatibility/rest-api/#apiv1sqlexecute---execute-queries) of the REST API.
