...
or [Implicit](https://openid.net/specs/openid-connect-core-1_0.html#ImplicitFlowAuth) flows
...
If Jupyter notebooks are used without JupyterHub, one option for OAuth2 integration is to
use the [Resource Owner Password Credentials](https://oauth.net/2/grant-types/password) flow.
It is likely that to enable this flow in your OAuth2 provider will require additional setup. The
Resource Owner Password Credentials flow is legacy, and should be used as a last resort.
...
The above example saves the user's credentials into the notebook, potentially exposing them to others.
One way to improve this is to use environment variables or files to externalize the username and password.
...
When using CLI tools, such as `psql`, or standalone applications like
Microsoft Access, the best option may be the Resource Owner Password Credentials flow.
...
Non-interactive clients are usually jobs, or standalone applications, for example, a client for ingesting data.
It is practical to manage their credentials via OAuth2 provider too.

As seen in the Jupyter notebook examples, the clients can request a token thmeselves and then use that to authorise data ingestion.
```python
import json
import os
import requests
